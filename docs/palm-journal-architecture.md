...
Commercial mode is the only production mode under the owner-authorized business-status configuration. Every Palm Journal build must pass `python scripts/validate_active_service_state.py`; `scripts/validate_site.py` runs it automatically. Keep Journal copy documentary and non-diagnostic, and preserve label, scope, certainty, and outcome boundaries without reintroducing treatment-unavailable restrictions.

...
def main() -> int:
    ...
    if not DIST.exists():
        ...
        for phrase in ("Manage multiple palms?", "Request a Palm Portfolio Walkthrough", "Homeowners"):
            if phrase not in homepage:
                errors.append(f"homepage missing commercial/residential pathway: {phrase}")
        for phrase in ("Palm Portfolio Baseline", "Protection and Monitoring", "Palm Stewardship", "15–20 minutes"):
